...
// Cuts aus dem TS-Katalog extrahieren (slug + visualBrief je Objekt).
async function parseCuts() {
  const src = await readFile(CATALOG, 'utf8')
  const re = /slug:\s*'([^']+)',[\s\S]*?visualBrief:\s*'((?:[^'\\]|\\.)*)'/g
  const cuts = []
  let m
  while ((m = re.exec(src)) !== null) {
    cuts.push({ slug: m[1], brief: m[2].replace(/\\'/g, "'") })
  }
  return cuts
}
...
function buildPrompt(brief) {
  return `${brief}, a single isolated cut of raw meat centered in frame on a dark charcoal slate surface, `
    ...
    + `fresh deep-red meat with natural marbling and texture, crisp focus, subtle shadow, `
...
async function generate(prompt) {
  const res = await fetch('https://fal.run/fal-ai/flux-lora', {
    ...
    body: JSON.stringify({
      ...
      loras: [{ path: FOODSTYLE_LORA, scale: 0.45 }],
    }),
  })
