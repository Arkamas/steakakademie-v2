...
| KI, **visuell klassifiziert** (Konfidenz überwiegend hoch) | 67 | 4 unabhängige Bildforensik-Agenten |
...
| **→ Fehlende `imageAI: true`-Kennzeichnung** | **111** | |
...
**Nachtrag 18.08.2026:** `schwertfisch-steak-grill` fehlte in der ursprünglichen Liste und ist nachgetragen (deshalb 67 statt 66). Aufgefallen beim Tagging-Lauf: Es war nach Abschnitt A und B das einzige Rezept ohne Zuordnung. Es kam im selben Commit wie Makrele, Sardinen, Mahi-Mahi und Rotbarbe (`cdf966b`), die alle in dieser Liste stehen, und zeigt dieselbe Signatur — Feuer-Bokeh, gestempelte Rauten, 1024×768, Metadaten entfernt. Keine C2PA-Belege, daher Abschnitt B.
